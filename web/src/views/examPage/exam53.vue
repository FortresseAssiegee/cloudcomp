<template>
  <v-container>
    ...
    <v-row class="d-flex h-100 mt-md-n16 pb-0">
        ...
        <v-card class="card-shadow px-4 py-4 overflow-hidden border-radius-xl">
          <v-row>
            ...
            <v-col cols="12" class="d-flex justify-center">
              该题还有：
              <br />
              <v-chip x-large color="purple" dark class="text-center" outlined>
            ...
            <v-col
              cols="12"
              class="my-sm-auto ms-sm-auto me-sm-0 mx-auto mt-3 d-flex justify-center"
            >
              <div class="see">
                <video
                  id="testVi"
      ...
      <v-col cols="9">
        <v-card class="mb-12" v-if="paTimeShow">
          <v-app-bar dark color="deep-purple darken-2">
            <v-app-bar-nav-icon></v-app-bar-nav-icon>
            <h5 class="mb-1 text-h5 font-weight-bold">考题</h5>
          </v-app-bar>
          <v-card-text>
            <v-row>
              <v-col cols="12">
                <v-card-title
                  >{{ nowExam + 1 }}.{{ examList[nowExam].title }}</v-card-title
                >
              </v-col>
              <v-col cols="6">
                <v-radio-group v-model="examList[nowExam].answer" column>
                  <v-radio
                    :label="n"
                    :value="n"
                    v-for="(n, num) in examList[nowExam].quest"
                    :key="num"
                  ></v-radio>
                </v-radio-group>
              </v-col>
              <v-col cols="6">
                <v-img :src="examList[nowExam].img"></v-img>
              </v-col>
            </v-row>
          </v-card-text>
          <v-card-actions
            ><v-btn
              @click="next()"
              v-if="nowExam != examList.length - 1"
              color="deep-purple lighten-2"
              text
              >下一题
            </v-btn>
            <v-spacer> </v-spacer>
            <v-btn @click="endExam()" color="deep-purple lighten-2" text
              >结束考试
            </v-btn></v-card-actions
          >
        </v-card>
        <v-card max-width="600" min-width="500" v-else>
          <v-app-bar dark color="deep-purple darken-2">
            <v-app-bar-nav-icon></v-app-bar-nav-icon>
            <h5 class="mb-1 text-h5 font-weight-bold">考前提示</h5>
          </v-app-bar>
          <v-card-text>
            <div>考试前请检查周围环境</div>
            <p class="text-h4 text--primary">距离考试还有{{ paTime }}秒</p>
            <p>诚信考试</p>
            <div class="text--primary">预祝你考试成功</div>
          </v-card-text>
          <v-card-actions>
            <v-btn color="orange lighten-2" text @click="$router.back()">
              退出考试
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
      ...
      that.axios
        ...
        .then((res) => {
          ...
          if (res.data.state == "win") {
            ...
            that.setInfo(that.examList[0].during);
    ...
    endExam() {
      ...
      that.axios
        ...
        .then((res) => {
          ...
          if (res.data.state == "win") {
            this.$refs.msg.setMsg(res.data.commont, "success");
    ...
    setInfo(during) {
      ...
      //到计时 during min *60
      that.maxtime = during; //s
      that.time = setInterval(function () {
        if (that.maxtime >= 0) {
          let minutes = Math.floor(that.maxtime / 60);
          let seconds = Math.floor(that.maxtime % 60);
          that.minNum = minutes;
          that.senNum = seconds;
          // let msg = minutes + "分" + seconds + "秒";
          // that.number = msg;
          if (that.maxtime == 5) {
            that.$refs.msg.setMsg("还有5秒！", "warning");
          }
    ...
    next() {
      ...
      setTimeout(() => {
        that.nowExam++;
        if (
          that.examList[that.nowExam].answer ==
          that.examList[that.nowExam].userAnswer
        ) {
          clearInterval(that.time);
          that.score += that.examList[that.nowExam].score;

          that.setInfo(that.examList[that.nowExam].during);
    ...
    fnImgRun() {
      ...
      that.uploadImg(data);
      // oss://cloud-compe/examIng/470/1668690016106

      // var params = {
      //   Project: "cloudcompe",
      //   ImageUriA: "oss://cloud-compe/avt/zxn.jpg",
      //   ImageUriB: geturl,
      // };
      ...
      // this.clientCore.request("CompareImageFaces", params, requestOption).then(
      //   (result) => {
      //     console.log("clientCore", JSON.stringify(result));
      //   },
      //   (ex) => {
      //     console.log(ex);
      //   }
      // );
      // console.log("simOSS obj：", {
      //   actvId: that.unitInfo.actvId,
      //   unitId: that.unitInfo.unitId,
      //   userCreateId: that.unitInfo.userCreateId,
      //   userJoinId: that.unitInfo.userJoinId,
      //   imageUriA: "oss://cloud-compe/gradeSelf/" + that.unitInfo.userJoinId,
    ...
    paStart() {
      ...
      let time = setInterval(function () {
        if (that.paTime > 0) {
          ...
          clearInterval(time);
          that.$refs.msg.setMsg("考试开始！", "info");
          that.paTimeShow = true;
          that.startExam();
        }
      }, 1000);
    },
