...
type GetRankHomeLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetRankHomeLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetRankHomeLogic {
	return &GetRankHomeLogic{
...
func (l *GetRankHomeLogic) GetRankHome(req *types.GetRankHomeReq) (resp *types.GetRankHomeResp, err error) {
	...
	return &types.GetRankHomeResp{
		Commont:         "获取成功",
		State:           "win",
		WeekHotRank:     HotList[:5],
		WeekLookUpRank:  LookupList[:5],
		SchoolCompeRank: UserList[:5],
	}, nil
}
